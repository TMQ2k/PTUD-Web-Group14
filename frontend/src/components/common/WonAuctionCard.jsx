...
const WonAuctionCard = ({ product, onRatingSuccess }) => {
  ...
  return (
    <article
      className={`border rounded-xl p-4 sm:p-5 flex flex-col gap-4 transition-shadow hover:shadow-md
        ${
          isCancelled
            ? "bg-red-50/50 border-red-200"
            : "bg-linear-to-br from-gray-50 to-blue-50/30 border-gray-200"
        }`}
    >
      {/* Top Section: Image + Details + Price */}
      <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
        {/* Product Image Container */}
        <div className="shrink-0 w-full sm:w-36 h-36 bg-gray-200 rounded-lg overflow-hidden relative border border-gray-100 shadow-xs">
          {product.image ? (
            <img
              src={product.image}
              alt={product.name}
              ...
              onError={(e) => {
                ...
                e.target.src = "https://placehold.co/150x150?text=No+Image"; // Fallback image
              }}
            />
          ) : (
        ...
        <div className="flex flex-1 flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          ...
          <div className="flex-1">
            {/* Cancelled Label */}
            {isCancelled && (
              <span className="inline-block mb-2 px-2.5 py-1 text-xs font-semibold text-red-700 bg-red-100 rounded-md border border-red-200">
                Giao dịch bị huỷ
              </span>
            )}
            ...
            {!isCancelled && product.sellerQrUrl && (
              <a
                href={product.sellerQrUrl}
                target="_blank"
                rel="noopener noreferrer"
          ...
          <div className="text-left sm:text-right mt-2 sm:mt-0">
            ...
            {/* Conditional Price Styling */}
            <p
              className={`text-lg sm:text-xl font-bold mt-0.5 ${
                isCancelled
                  ? "text-red-600 line-through decoration-2 decoration-red-600/50"
      ...
      {/* Vùng đánh giá & nhận xét (Chỉ hiện khi chưa huỷ) */}
      {!isCancelled && (
        <form
          onSubmit={handleSubmit}
          className="flex flex-col gap-3 sm:gap-4 border-t border-gray-200 pt-4 sm:pt-5 mt-1"
        >
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <p className="text-sm text-gray-700 font-semibold">
              Đánh giá người bán (bắt buộc):
