  ...
  ({
    ...
    return (
      <>
        ...
        <div
          className={twMerge(
            // Changed h-40 to h-fit to allow growth, added min-h to maintain shape
        ...
        >
          <div className="absolute left-0 top-0 h-full w-1.5 bg-gradient-to-b from-blue-400 to-purple-600"></div>

          {/* Left Side: Product Image 
              FIX: Removed 'h-full', added 'min-h-40'. 
              The div will stretch automatically due to flexbox. 
          */}
          <div className="w-40 min-h-40 shrink-0 relative rounded-xl overflow-hidden bg-gray-100 border border-gray-100">
            <img
              src={productImage}
              alt={productName}
              // FIX: Used absolute positioning to force fill the stretched container
              className="absolute inset-0 h-full w-full object-cover transition-transform duration-500 hover:scale-110"
            />
            <div className="absolute top-0 left-0 bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded-br-lg uppercase tracking-wider shadow-sm z-10">
              Won
            </div>
          </div>
          ...
          <div className="flex flex-1 flex-col justify-center ml-6 pr-4 py-2 min-w-0">
            <div>
              <h3 className="text-lg font-bold text-gray-900 leading-tight line-clamp-2 mb-2" title={productName}>
                {productName}
              </h3>
              <div className="h-0.5 w-12 bg-blue-200 rounded-full mb-3"></div>
            </div>
          ...
          {/* Ensure this column has enough width (w-72) to prevent squeezing */}
