...
function getProgress(start, end) {
  ...
  const progress =
    dateSubtraction(start_date, current_date) /
    dateSubtraction(start_date, end_date);
...
function remainingTime(ending_date) {
  ...
  const remaining_milliseconds = dateSubtraction(
    now,
    ending_date_to_millseconds
  );
