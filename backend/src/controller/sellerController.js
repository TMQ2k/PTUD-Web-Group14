...
router.post("/seller-reject-bidder", authenticate, async (req, res) => {
  try {
...
router.post("/seller-allow-bidder", authenticate, async (req, res) => {
  try {
    const { productId, bidderId } = req.body;
    const sellerId = req.user.id;
...
router.get("/all-requests/:id", authenticate, async (req, res) => {
  try {
    const sellerId = req.user.id;
