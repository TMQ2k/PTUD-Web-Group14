...
export const getBidHistoryByProductId = async (productId) => {
    const result = await pool.query(`SELECT * FROM auto_bids WHERE product_id = $1 ORDER BY created_at DESC`, [productId]);
    for (let row of result.rows) {
        const bidderResult = await pool.query(`SELECT u.username, ui.avatar_url FROM users u JOIN users_info ui ON u.user_id = ui.user_id WHERE u.user_id = $1`, [row.user_id]); 
        row.username = bidderResult.rows[0].username;
        row.avatar_url = bidderResult.rows[0].avatar_url;
    }
    return result.rows.map(row => new BidHistory(
        row.created_at,
        row.username,
        row.avatar_url,
