...
export const getProductsListofBidder = async (bidderId, limit, page, sortBy, is_active) => {
  try {
    const bidderProducts = await getProdudctsListByBidderIdRepo(bidderId, limit, page, sortBy, is_active);
    ...
    console.error("❌ [Service] Lỗi khi lấy danh sách sản phẩm của bidder:", err);
    throw err;
  }
}

export const getMetaDataForBidderProductsList = async (bidderId, limit, page, is_active) => {
  try {
    ...
    console.error("❌ [Service] Lỗi khi lấy metadata cho danh sách sản phẩm của bidder:", err);
    throw err;
  }
}
